...
uint8_t vibint=0;
bool lights_enabled = true;
uint8_t target_colors[NUM_LEDS][3];  // New array: where we WANT each LED to go
uint8_t current_colors[NUM_LEDS][3]; // Current shown color for each LED
  ...
  if (!received_colors && !received_brightness) {
    ...
    for (int i = 0; i < NUM_COLORS; i++) {
      for (int j = 0; j < section; j++) {
        int index = i * section + j;
        if (index < NUM_LEDS) {
          target_colors[index][0] = colors[i][0] * 0.300;  // Set TARGET R
          target_colors[index][1] = colors[i][1] * 0.300;  // Set TARGET G
          target_colors[index][2] = colors[i][2] * 0.300;  // Set TARGET B
        }
      }
    }
} else if (received_brightness) {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    for (int c = 0; c < 3; c++) {  // 0=R, 1=G, 2=B
      if (current_colors[i][c] < target_colors[i][c]) {
        current_colors[i][c]++;  // Move up by 1
      } else if (current_colors[i][c] > target_colors[i][c]) {
        current_colors[i][c]--;  // Move down by 1
      }
    }
