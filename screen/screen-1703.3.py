...
def show_grid_colors(colors, selected_colors):
    """Displays the detected colors in an OpenCV window with an LED color bar."""
    if not colors:
    ...
    color_grid = np.zeros((img_height + led_bar_height, img_width, 3), dtype=np.uint8)
    ...
    cv2.putText(color_grid, "Colors Sent to LED", (10, img_height + 15), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1, cv2.LINE_AA)

    # Display LED colors in a separate section
    led_block_width = img_width // NUM_DISTINCT_COLORS
    for i, color in enumerate(selected_colors):
        r, g, b = color["R"], color["G"], color["B"]
        led_x1, led_x2 = i * led_block_width, (i + 1) * led_block_width
        color_grid[img_height + 20:img_height + led_bar_height, led_x1:led_x2] = (b, g, r)
