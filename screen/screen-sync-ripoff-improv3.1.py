...
def show_grid_colors(colors, selected_colors):
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            # Mark selected LED colors with an "X" in an opposite color
            for (sr, sc, sr_r, sr_g, sr_b) in selected_positions:
                if sr == row and sc == col:
                    opposite_color = (255 - sr_b, 255 - sr_g, 255 - sr_r)  # Invert for contrast
                    cv2.line(color_grid, (x1, y1), (x2, y2), opposite_color, 2)  # Diagonal line
                    cv2.line(color_grid, (x1, y2), (x2, y1), opposite_color, 2)  # Other diagonal
