...
set "CONTROLLER_DIR=%CONTROLLER_DIR:~0,-1%"
...
if not exist "%CHECK_VB_SETUP%" (
    echo ❌ check_vb_install.py not found in: %CONTROLLER_DIR%
    pause
    exit /b 1
)

REM === Run VB-Cable check ===
...
REM === start /B "" cmd /k "%PYTHON_EXE% %BACKEND_PY%"
