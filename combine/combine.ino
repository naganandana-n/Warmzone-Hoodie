...
void loop() {
    if (Serial.available()) {
        StaticJsonDocument<512> doc;
        String input = Serial.readStringUntil('\n');  // Read JSON Data
        Serial.flush(); // 🛠 Flush serial buffer to prevent data corruption
        ...
        // **Process Screen Colors**
        if (doc.containsKey("LEDColors")) {
            int received_colors = doc["LEDColors"].size();
            if (received_colors > 0) {  // 🛠 Ensure colors exist
                for (int i = 0; i < min(received_colors, NUM_COLORS); i++) {
                    colors[i][0] = doc["LEDColors"][i]["G"]; // Swap Green
                    colors[i][1] = doc["LEDColors"][i]["R"]; // Swap Red
                    colors[i][2] = doc["LEDColors"][i]["B"]; // Keep Blue the same
                }
...
/** 🛠 Updates LED brightness based on audio intensity */
void updateAudioLEDs() {
    strip.clear(); // 🛠 Ensure previous colors don't interfere
    strip.setBrightness(led_brightness); // 🛠 Set before setting pixel color
...
void updateScreenLEDs() {
    strip.clear(); // 🛠 Ensure previous colors don't interfere
    strip.setBrightness(BRIGHTNESS); // 🛠 Ensure brightness consistency
